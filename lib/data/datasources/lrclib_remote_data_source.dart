  ...
  Future<LyricsModel> getLyrics(String trackName, String artistName) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (response.data is List) {
          ...
          if (results.isNotEmpty) {
            ...
            final validResult = results.firstWhere(
              (item) =>
                  ...
                  (item['plainLyrics'] != null || item['syncedLyrics'] != null),
            ...
            if (validResult != null) {
              ...
              throw Exception(
                  'No lyrics content found for this song on LrcLib (empty lyrics).');
            }
          } else {
            // Empty list means no lyrics found
            throw Exception(
                'No lyrics found for this song on LrcLib (empty list).');
          }
          ...
          if (dataMap.containsKey('error') && dataMap['error'] != null) {
            throw Exception(
                'Failed to get lyrics from LrcLib: ${dataMap['error']}');
          } else if (dataMap.containsKey('id')) {
            // Assuming 'id' means it's a valid lyrics object
            return LyricsModel.fromJson(dataMap);
          } else {
            throw Exception(
                'Unexpected response format from LrcLib (Map without error or id).');
          }
        } else {
          // If the response is not a list or a map, or is an unexpected format
          throw Exception(
              'Unexpected response format from LrcLib. Data: ${response.data}');
        }
      } else if (response.statusCode == 404) {
        // LrcLib might return 404 if no track is found at all.
        throw Exception(
            'No lyrics found for this song on LrcLib (404 Not Found).');
      } else {
        throw Exception(
            'Failed to get lyrics from LrcLib: Status ${response.statusCode}, Data: ${response.data}');
      }
    } on DioException catch (e) {
      String errorMessage =
          'Failed to get lyrics from LrcLib (DioError): ${e.message}';
      if (e.response != null) {
      ...
      if (e.type == DioExceptionType.connectionTimeout ||
        ...
        errorMessage =
            'Network timeout when fetching lyrics. Please check your connection.';
      }
