...
import 'package:lyricapture/domain/usecases/capture_lyrics_to_image.dart'; // Import use case
// import 'package:lyricapture/domain/repositories/lyrics_repository.dart'; // No longer needed if use cases are used
import 'package:screenshot/screenshot.dart';
// image_gallery_saver and share_plus might be used by the repository/usecase now
// import 'package:image_gallery_saver/image_gallery_saver.dart';
// import 'package:share_plus/share_plus.dart';
import 'package:permission_handler/permission_handler.dart'; // Keep for permission logic in provider
...
class LyricsProvider extends ChangeNotifier {
  ...
  final CaptureLyricsToImage _captureLyricsToImage; // Added CaptureLyricsToImage use case
  // final LyricsRepository _lyricsRepository; // Removed
  ...
  Future<String?> captureAndSaveLyrics(String songTitle, String artistName) async {
    ...
    if (status.isGranted) {
      try {
        ...
        if (imageBytes != null) {
          final fileName = "${songTitle.replaceAll(' ', '_').replaceAll(r'[^\w\s]+','_')}_lyrics.png";
          // Use the injected use case to save the image
          final String filePath = await _captureLyricsToImage.saveImage(imageBytes, fileName);

          // Use the injected use case to share the image
          await _captureLyricsToImage.shareImage(filePath, 'Lyrics for $songTitle by $artistName');
